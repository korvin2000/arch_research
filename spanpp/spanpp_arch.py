...
class Conv3XC(nn.Module):
    def __init__(
        self, c_in: int, c_out: int, gain1: int = 2, s: int = 1, bias: bool = True
    ) -> None:
        super().__init__()
        self.bias = bias
...
class EvalConv(nn.Module):
    def __init__(self, weight, kernel_size) -> None:
        super().__init__()
        self.weight = weight
    ...
    def forward(self, x):
        return F.conv2d(
            x, self.weight.to(x), bias=None, stride=1, padding=self.kernel_size // 2
        )
...
class IGConv(nn.Module):
    def __init__(
        ...
        self.freq = nn.Parameter(
            torch.randn((dim * kernel_size**2), implicit_dim, 1, 1) * 0.02,
            requires_grad=True,
        )
    ...
    def train(self, mode: bool = True) -> Self:
        with torch.no_grad():
            for scale in self.scale_list:
                self.eval_convs[str(scale)] = EvalConv(
                    self.resize(scale), self.kernel_size
                )
    ...
    def forward(self, x, scale):
        ...
        if self.training:
            ...
            rgb = F.conv2d(
                x, k_interp, bias=None, stride=1, padding=self.kernel_size // 2
            )
        else:
    ...
    def _implicit_representation_latent(self, scale):
        ...
        r = (
            torch.ones(1, 1, scale, scale).to(self.query_kernel[0].weight.device)
            / scale_phase
            * 2
        )  # 2 / r following LIIF/LTE
        coords = (
            make_coord((scale, scale))
            .unsqueeze(0)
            .to(self.query_kernel[0].weight.device)
        )
        ...
        freq = freq + phase  # RGB
        freq = torch.cat(
            [torch.cos(torch.pi * freq), torch.sin(torch.pi * freq)], dim=1
        )  # cos(R)cos(G)cos(B) sin(R)sin(G)sin(B)
