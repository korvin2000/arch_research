...
class Upsampling(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.conv = nn.Conv2d(in_channel, 1, 3, 1, 1, bias=False)

    def forward(self, x, x_ori):
        x = self.upsample(x)
        x = torch.sigmoid(self.conv(x)) * x_ori
        return x
    
class SequenceTraversal(Enum):
    ROWWISE_FROM_TOP_LEFT = "rowwise_from_top_left"
    ROWWISE_FROM_BOT_RIGHT = "rowwise_from_bot_right"
    ROWWISE_FROM_TOP_LEFT_transpose = "rowwise_from_top_left_transpose"
    ROWWISE_FROM_BOT_RIGHT_transpose = "rowwise_from_bot_right_transpose"
    ROWWISE_FROM_TOP_LEFT_RG = "rowwise_from_top_left_RG"
    ROWWISE_FROM_BOT_RIGHT_RG = "rowwise_from_bot_right_RG"
    HILBERTCURVE = "hilbertcurve"
    HILBERTCURVE_flip = "hilbertcurve_flip"
...
class ViLLayer(nn.Module):
    ...
    def _generate_hilbert_indices(self, H, W, device):
        assert H == W and (H & (H - 1)) == 0
        n = int(math.log2(H))
        
        if (H, W) not in self.hilbert_cache:
            hilbert_curve = HilbertCurve(n, 2)
            indices = []
            for i in range(H * W):
                x = i % W
                y = i // W
                distance = hilbert_curve.distance_from_point([x, y])
                indices.append((distance, i))
            
            indices.sort()
            hilbert_indices = torch.tensor([idx for _, idx in indices], device=device)
            inverse_indices = torch.empty_like(hilbert_indices)
            inverse_indices[hilbert_indices] = torch.arange(H*W, device=device)
            self.hilbert_cache[(H, W)] = (hilbert_indices, inverse_indices)
        
        return self.hilbert_cache[(H, W)]

    def forward(self, x: torch.Tensor, resolution) -> torch.Tensor:
        B, S, C = x.shape
        H, W = resolution
        ...
        if self.direction == SequenceTraversal.ROWWISE_FROM_TOP_LEFT:
            ...
            x = x.flip(dims=[1])
        elif self.direction == SequenceTraversal.ROWWISE_FROM_TOP_LEFT_transpose:
            x = x.view(B, H, W, C)    
            x = x.transpose(1, 2)      
            x = x.reshape(B, S, C)     
        elif self.direction == SequenceTraversal.HILBERTCURVE:
            if H * W != S or H != W or (H & (H - 1)) != 0:
                raise ValueError(f"The input sequence length {S} must be a perfect square and a power of 2, such as 64 (8x8), 256 (16x16).")
            hilbert_indices, inverse_indices = self._generate_hilbert_indices(H, W, x.device)
            x = x[:, hilbert_indices, :]
        else:
            raise NotImplementedError
...
class ViLBlock(nn.Module):
    def __init__(
        ...
        self.Down = WaveletDownsampling(dim,dim)
        self.Up = Upsampling(dim,dim)
        self.reset_parameters()

    def _forward_path(self, x, resolution):
        x = self.norm(x)
        x = self.layer(x, resolution)
        return x
    ...
    def forward(self, x: torch.Tensor, resolution) -> torch.Tensor:
        ...
        resolution = (int(h/2), int(w/2))

        x = self.drop_path(x, self._forward_path, {'resolution': resolution})
...
class ViLBlockPair(nn.Module):
    def __init__(
        ...
        self.HILBERTCURVE = ViLBlock(
            dim=dim,
            direction=SequenceTraversal.HILBERTCURVE,
            drop_path=drop_path,
            conv_kind=conv_kind,
            conv_kernel_size=conv_kernel_size,
            proj_bias=proj_bias,
    ...
    def forward(self, x, resolution):
        x = self.rowwise_from_top_left(x, resolution)
        x = self.rowwise_from_bot_right(x, resolution)
        x = self.HILBERTCURVE(x, resolution)
        return x
